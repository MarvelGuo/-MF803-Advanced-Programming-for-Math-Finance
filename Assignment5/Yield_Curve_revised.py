...
class Yield_Curve:
    ...
    def pricing_func(self, rf, D, T, diff_T):
        ...
        :param rf: forward rate
        :param D: Discount Factor (list)
        :param T: maturity
        :param diff: gaps between current year and last given year
        '''
    ...
    def cal_ZeroRates(self):
        temp = [-np.log(d) * 2 / (i + 1) for i, d in enumerate(self.Discount)]
        return list(map(lambda x: 2 * (exp(0.5 * x) - 1), temp))
    ...
    def breakeven_swap(self, Y):
        ...
        for y in np.linspace(0.5, Y, 2 * Y):
            ...
            F = 2 * (exp(0.5 * self.FwR[forward_year]) - 1)
            float_legs += 0.5 * \
                F * self.Discount[int(2 * y - 1)]
